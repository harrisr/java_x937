...
/**
 * The Class FileIDModifierValidator.
 */
public class FileIDModifierValidator extends Validator {
    public static final FileIDModifierValidator INSTANCE = new FileIDModifierValidator();
    ...
    public ValidationResult validateData(String data) throws ICLException {
        ...
        if (data.toUpperCase().equals(data)) {
            result.setMessage("File ID Modifier is valid");
            result.setDataValid(true);
        } else {
            result.setMessage("File ID Modifier [" + data + "] did not pass validation check!");
